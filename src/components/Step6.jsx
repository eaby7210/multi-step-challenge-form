...
const ACCESS_OPTIONS = [
  {
    ...
    prompt: [
      ...
      {
        name: "lock_box_location",
        label: "Location/Directions",
        type: "text",
        placeholder: "Describe location or directions",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    prompt: [
      {
        name: "hidden_key_directions",
        label: "Directions",
        type: "text",
        placeholder: "Where is the key hidden?",
      },
    ],
  },
  {
    ...
    prompt: [
      {
        name: "community_access_instructions",
        label: "Instructions",
        type: "text",
        placeholder: "Gate code, guard, etc.",
      },
    ],
  },
  {
...
const Step6 = ({ formData, handleChange, onPrev, onNext }) => {
  ...
  const validateForm = () => {
    ...
    if (
      ...
      !["vacant", "occupied"].includes(formData.occupancy_status)
    ) {
        ...
        option.prompt.forEach((field) => {
          if (option.key === "meet_contact" && field.name === "contact_email") {
            if (
              formData[field.name] &&
              field.type === "email" &&
              !emailRegex.test(formData[field.name])
            ) {
              errors[
                field.name
        ...
        <div className="mb-10 flex flex-col items-center">
            ...
            <label
              ...
              ${
                formData.occupancy_status === "vacant"
                  ...
                  : "border-gray-200 hover:border-primary hover:label-active-gradient hover:text-inverse"
              }`}
            >
              <input
                type="radio"
          ...
          {hasAttemptedNext && validationErrors.occupancy_status && (
            <div className="text-red-500 text-sm mt-2 text-center">
              {validationErrors.occupancy_status}
            </div>
          )}
        </div>
        {/* Access Options */}
